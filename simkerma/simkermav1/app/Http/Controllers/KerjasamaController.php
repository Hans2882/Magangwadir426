...
{
    /**
     * Generic server-side DataTables handler for dokumen_kerjasama tables.
     *
     * @param  string  $jenisDokumen  e.g. 'PKS', 'IA'
     */
    private function dalamNegeri(Request $request, string $jenisDokumen)
    {
        ...
        $query = DB::table('dokumen_kerjasama as dk')
            ...
            ->where('dk.jenis_dokumen', $jenisDokumen)
        ...
        $total = DB::table('dokumen_kerjasama')->where('jenis_dokumen', $jenisDokumen)->count();
    ...
    private function luarNegeri(Request $request, string $jenisDokumen)
    {
        $draw     = $request->input('draw', 1);
        $start    = (int) $request->input('start', 0);
        $length   = (int) $request->input('length', 10);
        ...
        $query = DB::table('dokumen_kerjasama_ln as dln')
            ...
            ->where('dln.jenis_dokumen', $jenisDokumen)
        ...
        $total = DB::table('dokumen_kerjasama_ln')->where('jenis_dokumen', $jenisDokumen)->count();
    ...
    // ── PKS ──────────────────────────────────────────────
    public function pksIndex()  { return view('data-kerjasama.data-pks'); }
    public function pksDN(Request $r) { return $this->dalamNegeri($r, 'PKS'); }

    // ── IA ───────────────────────────────────────────────
    public function iaIndex()   { return view('data-kerjasama.data-ia'); }
    public function iaDN(Request $r)  { return $this->dalamNegeri($r, 'IA'); }
    public function iaLN(Request $r)  { return $this->luarNegeri($r, 'IA'); }
}
